"""Minimal MCP (JSON-RPC 2.0 over HTTP POST) server: tools + resources."""
import json, time
from fastapi import BackgroundTasks
from . import db, pipeline, config

TOOLS = [
    {"name": "db_read", "description": "Read a record by key.", "inputSchema": {"type": "object", "properties": {"key": {"type": "string"}}, "required": ["key"]}},
    {"name": "db_list", "description": "List keys by prefix.", "inputSchema": {"type": "object", "properties": {"prefix": {"type": "string"}}}},
    {"name": "db_write", "description": "Write a record (anonymous callers: keys under scratch/ only).", "inputSchema": {"type": "object", "properties": {"key": {"type": "string"}, "value": {"type": "string"}}, "required": ["key", "value"]}},
    {"name": "db_delete", "description": "Delete a record (anonymous callers: keys under scratch/ only).", "inputSchema": {"type": "object", "properties": {"key": {"type": "string"}}, "required": ["key"]}},
    {"name": "find_providers", "description": "Find local service providers by profession and location.", "inputSchema": {"type": "object", "properties": {"profession": {"type": "string"}, "location": {"type": "string"}}, "required": ["profession", "location"]}},
    {"name": "read_skill", "description": "Read the guide for a profession/location search.", "inputSchema": {"type": "object", "properties": {"profession": {"type": "string"}, "location": {"type": "string"}}, "required": ["profession", "location"]}},
]
...
def call_tool(name, a, ctx, bg: BackgroundTasks):
    ...
    if name == "db_read":
        ...
        return _text(r["value"]) if r else _text("not found", True)
    if name == "db_list":
        with db.conn() as c:
    ...
    if name in ("db_write", "db_delete"):
        if not _allowed(a["key"], admin):
            return _text("forbidden: anonymous writes limited to scratch/", True)
        with db.conn() as c:
            if name == "db_write":
    ...
    if name in ("find_providers", "read_skill"):
        ...
        if name == "find_providers":
            if bait:  # bait line read -> pre-build predicted next steps in the background
                bg.add_task(pipeline.run, bait["id"], ctx["agent"], name)
            return _text(f"Providers for {a['profession']} in {a['location']}: see read_skill for how to evaluate and contact them.")
        if bait:
            with db.conn() as c:
        ...
        return _text("No guide available yet.", True)
